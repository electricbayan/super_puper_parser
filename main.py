...
def get_flats_data_from_db(cursor):
    ...
    {flat_id: {price: int, address: str, total_square: float, living_square: float, type: str, district: str, coords: str}}
    """
    d = {}
...
def get_shops_data_from_db(cursor):
    """
    ...
    for string in cursor.execute("""SELECT * FROM shops"""):
        d[string[0]] = {}
        d[string[0]]['coords'] = string[1].replace(';', ',')
